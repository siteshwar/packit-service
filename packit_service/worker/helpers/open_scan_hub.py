...
from packit_service.service.urls import get_copr_build_info_url, get_koji_build_info_url, get_openscanhub_info_url
...
class OpenScanHubHelper:
    ...
    def download_srpms(
        ...
        if (base_srpm_path := OpenScanHubHelper.download_srpm(base_srpm_model)) is None:
            return None

        if (srpm_path := OpenScanHubHelper.download_srpm(srpm_model)) is None:
            return None
...
class KojiOpenScanHubHelper(OpenScanHubHelper):
    ...
    def find_base_build_job_id(self) -> Optional[int]:
        ...
        (with `commit` trigger and same branch configured as the target PR branch).
        """
        # We should take this approach as Packit does not have a base build for every package in its database.
        koji_helper = koji_helper.KojiHelper()
        result = koji_helper.get_latest_build_in_tag(tag="rawhide", package=self.build.get_package_name())
